...
public class GetFlavorsDetailTest {

	@Test
	public void test() {
		Token token;
		GetFlavorsDetail getVersions = new GetFlavorsDetail();
		try {
			token = ConohaTokenManager.loadDefault();
			GetFlavorsDetailResponse res = getVersions.request(token, token.tenant.id);
