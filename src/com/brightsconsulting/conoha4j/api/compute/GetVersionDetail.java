...
public class GetVersionDetail extends APIInvoker {
	...
	GetVersionDetailResponse request(Version.Type type, Token token)
		...
		String url = EndPoints.ComputeService + "/" + type.getName();

		String json = this.getAPIClient().get(url, token);
		GetVersionDetailResponse resVersion = this.getObjectMapper().readValue(json, GetVersionDetailResponse.class);
		return resVersion;
